...
SELECT city.cityname, city.population/1000 AS "Population in thousands" FROM City, Country WHERE countryName = "Ethiopia" AND city.countryCode = country.countryCode;
...
SELECT country.countryname, country.countryCode, country.totalPopulation/country.area AS "Population density" FROM country WHERE countryCode LIKE "M%";
...
SELECT country.capital, country.area*0.39 AS "Area in square miles" FROM country, city WHERE city.countryCode = country.countryCode AND city.population > 2000000 AND country.area > 500000*0.39 GROUP BY country.capital ORDER BY country.capital ASC;
...
SELECT country.countryName, country.capital, ROUND(city.longitude / 15, 1) AS "Time difference from UK in hours" FROM country, city WHERE city.countryCode = country.countryCode AND city.longitude > 0 GROUP BY country.countryName ORDER BY city.longitude / 15 DESC, country.countryName ASC;
...
SELECT country.capital, ROUND((city.latitude - 51.5) * 113,2) AS "Distance north of London in KM" FROM country, city WHERE city.countryCode = country.countryCode AND city.longitude < 35 AND city.latitude >  51.5 GROUP BY country.capital ORDER BY country.capital ASC; 
